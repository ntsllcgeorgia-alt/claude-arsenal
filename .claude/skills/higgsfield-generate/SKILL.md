...
## Variation pattern — when the user wants choice

For first-pass exploration where the user hasn't locked the brief yet (cover slides, hero stills, ad creatives), generate **4 variations in parallel** instead of one. Two ways:
- `higgsfield generate create <model> --prompt "..." --count 4 --wait` if the model supports `--count`.
- Otherwise fire 4 parallel `create` calls in one Bash invocation (`&` them, then `wait`). Each gets its own random seed.

Cost-check before firing variations: `higgsfield generate cost <model> --prompt "..."` × 4. If the total > $0.50, surface to the user before running.

When using 4-variation flow for downstream style consistency (carousels, multi-slide sets), have the user pick a winner BEFORE generating the next slide — then pass the winner as `--image` reference for every subsequent slide. This is the visual-consistency trick: anchor all derivatives to one human-chosen frame, not each model call's default aesthetic.

## MCP path (alternative to CLI)

As of 2026-05-17 the official Higgsfield MCP is wired up at user scope (`https://mcp.higgsfield.ai/mcp`, OAuth on first call). When the MCP is connected in the current Claude Code session, native MCP tool calls (`mcp__higgsfield__generate_image`, `mcp__higgsfield__generate_video`, etc.) work in place of Bash-spawned `higgsfield generate create`. Same model catalog, same parameters, lower latency, no subprocess overhead. The CLI route remains the documented default for portability — both paths are valid; pick by what's available in the current session.
